...
This plugin uses a **single concurrency model**, meaning it processes events sequentially in a single thread. 
...
**Delivery guarantees**: TCP provides at-most-once delivery semantics. If the connection is lost or 
the remote endpoint is unavailable, events may be lost without retry. For guaranteed delivery, 
consider using a message queue or a plugin with built-in retry mechanisms.
