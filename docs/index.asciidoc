...
This plugin uses **single-threaded operation**, meaning it processes events sequentially in a single thread. 
The Logstash `workers` setting has no effect on this plugin's throughput. To increase TCP output capacity, 
consider deploying multiple Logstash instances or using a different output plugin.

**No batching**: This plugin does not batch events. Each event is sent individually to the TCP socket as it 
is received, which may impact throughput for high-volume scenarios.

**Delivery guarantees**: In `client` mode, the plugin automatically retries failed writes and connection 
attempts indefinitely. However, events may be lost if Logstash is terminated while retrying. In `server` 
mode, if a client connection fails, events sent to that specific client during the failure may be lost 
without retry. For guaranteed delivery across all scenarios, consider using a message queue or a plugin 
with built-in acknowledgment mechanisms.
